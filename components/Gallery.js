...
export default function Gallery(props) {
  const { imageList, setImage, handleImage, setEdit, edit } = props;
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,_250px)] grid-rows-auto justify-center  border-2 border-solid border-lime-500 ">
      {Object.keys(imageList).map((image, i) => {
        return (
          <div key={i}>
            <Thumbnail image={imageList[image]} handleAddEdit={handleAddEdit} edit={edit} />
          </div>
        );
      })}
      {!addingImage && (
