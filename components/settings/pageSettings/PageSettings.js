...
export default function PageSettings(props) {
  ...
  const { register, unregister, handleSubmit, watch, reset, getValues, formState: { errors, isDirty }, setValue } = useForm()
  ...
  if (!portfolioData) {
    ...
    return (
      <div>
        <div className="relative flex max-w-[80%] m-auto text-lg font-semibold [&_*]:basis-full">
          <button onClick={() => setCurrentForm('display')} className="p-2 hover:text-bgAccent duration-150">Display info</button>
        ...
        <form className="settings-form relative flex flex-col sm:p-10 p-3">
          ...
          {
            currentForm === 'socials' &&
            <SocialsForm title="socials" register={register} unregister={unregister} watch={watch} setValue={setValue} getValues={getValues} errors={errors}/>
          }
          {
            currentForm === 'description' &&
            <DescriptionForm title="description" register={register} watch={watch} setValue={setValue}/>
          }
          {
            currentForm === 'theme' &&
            <ThemeForm title="theme" register={register} watch={watch} setValue={setValue}/>
          }
          
          
          <button onClick={handleSubmit(onSubmit)} type="button" className="float-right ml-auto mt-5 border-2 border-bgAccent w-24 h-10 rounded text-lg hover:bg-bgAccent duration-150">
            {updateLoading ? <LoaderAnimation small={true}/> : (didUpload ? <i className="text-lime-600 fa-solid fa-check"></i> : "Submit")}
          </button>
        </form>
