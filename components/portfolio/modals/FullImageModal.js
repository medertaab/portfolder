...
export default function FullImageModal(props) {
  ...
  function handleNextClick(e) {
    ...
    if (nextImageIndex >= Object.keys(images).length) {
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    if (image) {
      router.push(`/${username}/${image.title}?id=${imageIndex}`, undefined, {
        shallow: true,
      });
    }
  }, [imageIndex]);
  ...
  return (
    <div className="fixed z-50 inset-0 max-h-full max-w-full bg-bgPrimary bg-opacity-80 backdrop-blur-md flex flex-col items-center justify-center">
      {/* Close modal button */}
      <Link
        href={`/${username}`}
      ...
      <div className="absolute flex z-40 justify-between w-full sm:max-w-5xl px-5">
        <button
          ...
          className="bg-bgPrimary text-textPrimary shadowy sm:text-6xl text-3xl bg-opacity-20 rounded-full h-10 w-10 sm:h-14 sm:w-14 sm:hover:bg-textPrimary sm:hover:bg-opacity-50 sm:hover:text-white duration-150 flex items-center justify-center"
        >
      ...
      <div
        className={`fade-in h-fit sm:h-3/4 flex items-center self-center`}
